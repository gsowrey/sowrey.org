jQuery(document).ready(function($) {
	...
	$(document).on('click', 'a.photonic-flow-more', function(e) {
		...
		if (link !== undefined && link !== '') {
			...
			parameters = 'action=photonic_wizard_more&provider=' + $clicked.data('photonicProvider') + '&display_type=' + $clicked.data('photonicDisplayType') + '&url=' + btoa($clicked.data('photonicMoreLink'));
			$.post(Photonic_Wizard_JS.ajaxurl, parameters, function(data){
				$clicked.hide();
